...
def ml_anomaly(pm25_json: str, contamination: float = 0.05):
    ...
    df = pd.read_json(pm25_json)
    df["datetime_utc"] = pd.to_datetime(df["datetime_utc"])
    df = df.sort_values("datetime_utc").dropna(subset=["pm25"]).reset_index(drop=True)
...
with tab2:
    ...
    if n_pm25_pts < 48:
        ...
        with st.spinner("Running ML models…"):
            fc_records_json, test_json, fc_metrics = ml_forecast(pm25_json)
        ...
        # ──────────────────────────────────────────────────────────
        st.markdown(
            '<p class="section-header">PM2.5 Forecast — Next 12 Hours</p>',
            unsafe_allow_html=True,
        )
        ...
        with col_info:
            ...
            st.markdown(
                ...
                <b>Horizon</b><br/>12 hours ahead<br/><br/>
            ...
            if fc_df is not None:
                ...
                fig_fc.update_layout(
                    **PLOTLY_LAYOUT_NO_LEGEND,
                    title=f"PM2.5 Forecast — {ml_station}",
        ...
        if fc_df is not None:
            with st.expander("Forecast Values (next 12h)", expanded=False):
                disp = fc_df.copy()
